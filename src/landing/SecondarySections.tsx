...
const FeatureCard = ({ icon: Icon, title, subtitle, steps, delay }: any) => (
    <motion.div
        ...
        className="group p-10 bg-[var(--surface)] border border-[var(--border)] rounded-[2.5rem] hover:border-[var(--accent)]/30 transition-all duration-500 shadow-sm"
    >
        <div className="w-16 h-16 bg-[var(--surface-highlight)] rounded-3xl flex items-center justify-center mb-10 group-hover:scale-110 group-hover:bg-[var(--accent)]/10 transition-all duration-500 border border-[var(--border)]">
            <Icon className="h-7 w-7 text-[var(--text-muted)] group-hover:text-[var(--accent)] transition-colors" />
        </div>
        ...
        <div className="space-y-6">
            {steps.map((step: string, i: number) => (
                <div key={i} className="flex gap-5 items-start text-[var(--text-muted)] group-hover:text-[var(--text-main)] transition-colors">
            ...
            <div className="relative flex overflow-hidden group">
                <div className="flex gap-8 animate-marquee whitespace-nowrap py-4">
                    {[...reviews, ...reviews, ...reviews].map((review, i) => (
                        <div key={i} className="w-[400px] p-8 bg-[var(--surface)] border border-[var(--border)] rounded-[2rem] flex-shrink-0 hover:border-[var(--accent)]/30 transition-colors">
    ...
    <footer className="py-20 px-6 border-t border-[var(--border)] bg-[var(--background)]">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12">
            <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-[var(--surface)] rounded-xl flex items-center justify-center border border-[var(--border)] overflow-hidden shadow-xl">
            ...
            <div className="flex flex-col items-center md:items-end gap-3">
                ...
                <span className="font-bold text-xs text-[var(--text-muted)] uppercase tracking-widest">© 2026 Labs</span>
            </div>
        </div>
    </footer>
);
