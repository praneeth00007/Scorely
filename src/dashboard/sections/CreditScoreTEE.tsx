...
import { Shield, ChevronRight, ChevronLeft, RotateCcw, X, CheckCircle, Play } from 'lucide-react';
...
const EXAMPLE_VALUES: DetailedFinancialData = {
  income: { annualSalaryUSD: 120000, otherIncomeUSD: 15000, employmentStabilityMonths: 60 },
  liabilities: { totalOutstandingDebtUSD: 12000, monthlyDebtPaymentUSD: 500 },
  creditUtilization: { totalCreditLimitUSD: 50000, currentUtilizedUSD: 2000 },
  creditHistory: { oldestAccountMonths: 120, averageAccountAgeMonths: 72, latePayments: { '30D': 0, '60D': 0, '90D': 0 } },
  creditMix: { creditCards: 5, installmentLoans: 1, mortgage: 1 },
  newCredit: { hardInquiriesLast12Months: 0, newAccountsLast12Months: 0 }
};
...
const CreditScoreTEE = () => {
  ...
  useEffect(() => {
    const storedHistory = localStorage.getItem('scora_history');
    if (storedHistory) {
      try {
  ...
  const handleEncryptAndRun = () => {
    ...
    try {
      localStorage.setItem(`scora_run_${runId}`, JSON.stringify(formData));
    } catch (e) {
      ...
      <section className="mb-12">
        <div className="border border-slate-800 bg-slate-900/40 rounded-lg p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
          ...
          <button
            ...
            className="bg-white text-slate-950 px-6 py-3 rounded-md font-medium text-sm hover:bg-slate-200 transition-colors flex items-center gap-2 whitespace-nowrap"
          >
          ...
          <div className="bg-[#0f1116] border border-slate-700 w-full max-w-2xl rounded-lg shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-center justify-between p-6 border-b border-slate-800">
              ...
              <div className="flex items-center gap-4">
                <button
                  ...
                  className="text-xs font-bold text-blue-400 hover:text-blue-300 transition-colors uppercase tracking-tight flex items-center gap-1.5"
                >
            ...
            <div className="p-6 border-t border-slate-800 bg-[#0c0e12] rounded-b-lg flex items-center justify-between mt-auto">
              <button
                ...
                className="text-slate-400 hover:text-white disabled:opacity-30 disabled:hover:text-slate-400 flex items-center gap-1 text-sm font-medium transition-colors"
              >
              ...
              {currentStep < 5 ? (
                <button
                  ...
                  className="bg-white text-black px-6 py-2.5 rounded font-bold text-sm hover:bg-slate-200 transition-colors flex items-center gap-2"
                >
                ...
                <button
                  ...
                  className="bg-emerald-500 text-black px-6 py-2.5 rounded font-bold text-sm hover:bg-emerald-400 transition-colors flex items-center gap-2 shadow-lg shadow-emerald-900/20"
                >
