...
const Profile = () => {
    ...
    React.useEffect(() => {
        const stored = localStorage.getItem('scora_history');
        if (stored) {
            try {
            ...
            <div className="max-w-4xl mx-auto pb-24">
                ...
                <header className="mb-16">
                    ...
                    <div className="flex items-center gap-4">
                        <code className="font-mono text-sm text-slate-400 bg-slate-900 border border-slate-800 px-4 py-2 rounded select-all cursor-text hover:border-slate-700 transition-colors">
                            {address || '0x0000...0000'}
                        </code>
                                    ...
                                    history.map((item, index) => (
                                        <tr key={index} className="hover:bg-slate-900/30 transition-colors">
                                            <td className="px-6 py-4 text-slate-300 font-mono">
                                                <a
                                                    href={`https://explorer.iex.ec/bellecour/task/${item.taskId}`}
                                                    target="_blank"
