  ...
  return (
    ...
    >
      ...
      {!isLastStep && !isSkipping && (
        <button
          onClick={handleSkip}
          className="absolute top-6 left-8 z-20 text-[10px] font-black tracking-widest text-zinc-600 hover:text-white transition-colors uppercase"
        >
          Skip Tour
        </button>
      )}
      ...
      <div className="flex-1 flex flex-col p-8 md:p-10 min-h-[320px] pt-16">
        ...
        {isSkipping ? (
          <div className="flex flex-col items-center justify-center flex-1 animate-fade-in text-center h-full">
            ...
            <p className="text-zinc-400 text-sm">Thank you for exploring Scora.</p>
          </div>
        ) : (
          <div className="flex flex-col h-full">
            ...
            <div className="flex justify-end items-center mt-auto pt-4 border-t border-white/5 h-16 shrink-0">
              <div className="flex gap-4 items-center">
                {index > 0 && (
                  <button {...backProps} className="text-[10px] font-black tracking-widest text-[var(--border)] hover:text-white transition-colors uppercase">
                    Prev
                  </button>
                )}
                <button
                  {...primaryProps}
                  className="bg-[var(--accent)] hover:bg-blue-600 text-white text-xs font-bold py-3 px-8 rounded-2xl transition-all shadow-lg shadow-blue-500/20"
                >
  ...
  const defaultSteps: Step[] = [
    {
      target: 'body',
      placement: 'center',
      title: 'Scora Terminal Initialized',
      content: (
        <div className="flex flex-col gap-3">
          <p className="text-base text-zinc-200 font-medium">
            Welcome to the <span className="text-white font-bold">Scora Protocol</span>.
          </p>
    ...
    },
    {
      ...
      content: (
        <div className="flex flex-col gap-2">
          <p className="text-zinc-300">Your wallet <span className="text-emerald-500 font-bold">SGX Session</span> is active. You can terminate it here at any time.</p>
        </div>
      ),
    },
  ];
