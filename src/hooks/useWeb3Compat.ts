...
export function useAccount() {
    ...
    console.log("useAccount hook:", { wallet: !!wallet, connecting });
...
export function useDisconnect() {
    ...
    return useMemo(() => ({
        disconnect: async () => {
            if (wallet) {
                // Clear persistence
                localStorage.removeItem('scora_wallet_label');
...
export function useSessionRestore() {
    ...
    useEffect(() => {
        const label = localStorage.getItem('scora_wallet_label');
            ...
            connect({ autoSelect: { label, disableModals: true } })
                .then(async (state) => {
                    ...
                    if (state && state.length > 0) {
                        ...
                        if (currentChain !== '0x86') { // 134
                            console.log("Wrong chain detected. Switching to Bellecour...");
                            await setChain({ chainId: '0x86' });
                        }
                    }
                })
                .catch(err => {
    ...
    useEffect(() => {
        if (wallet?.label) {
            localStorage.setItem('scora_wallet_label', wallet.label);
        }
    }, [wallet]);

    // 3. Watch for chain changes during active session (optional, but good for safety)
    useEffect(() => {
        if (wallet && connectedChain && connectedChain.id !== '0x86') {
